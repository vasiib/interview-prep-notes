...
// 2. Impelement a retry mechanism for failed APIs
...
async function apiRequestWithRetry(url, maxRetries = 3, delay = 1000) {
  ...
  while (attempt < maxRetries) {
    try {
      ...
      attempt++;
      if (attempt === maxRetries) throw new Error('Max retries reached. Request failed.');
...
//app.get('/fetch-data', async (req, res) => {
  const apiUrl = 'https://example.comm/api/resource';
  try {
