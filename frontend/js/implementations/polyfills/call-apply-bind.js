...
Function.prototype.myBindSimple = function (newThisArg, ...boundArgs) {
  const bindCalledOn = this;

  return function (...argumentsFromNewCall) {
    const result = bindCalledOn.apply(newThisArg, [...boundArgs, ...argumentsFromNewCall]);
    return result;
  };
};
...
const myObject = {
  name: "Vaseem",
  getname: function (newParam) {
    return this.name + " " + newParam;
  },
};

const newObject = { name: "Shaik" };

const boundFunction = myObject.getname.myBindSimple(newObject);
console.log("vaseem testing", boundFunction("Vaseem")); // Output: "Shaik Vaseem"
